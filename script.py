    ...
    for ppty, ppty_dict in contents["properties"].items():
        ...
        md_file.new_list(
            [
                ...
                "can be null"
                if "null" in ppty_dict.get("type", "") or ppty_dict.get("nullable")
                else "can not be null",
            ]
        )
...
if __name__ == "__main__":
    ...
    root_path = Path("../../../rebate-api/json_schema_validator/resolved_json_schemas")
    export_path = Path("../../../IRA-Rebates-API/docs")

    index_md = MdUtils(file_name=str(export_path/ 'index.md'))
    index_md.new_header(level=1, title='IRA Rebate API JSON Schemas')
    link_items = []
    nav_items = []
    for file in root_path.iterdir():
        folder_name = re.sub(r"(?<!^)(?=[A-Z])", "_", file.name.split(".")[0]).lower()
        title  = re.sub(r'(?<!^)(?=[A-Z])', ' ', file.name.split(".")[0]).title()
        schema_folder = export_path / folder_name
        schema_folder.mkdir(exist_ok=True)
        generate_markdown_files(file, schema_folder)
    ...
    style_sheet_folder = export_path / "stylesheets"
    style_sheet_folder.mkdir(exist_ok=True)
    css_contents = """
    ...
    """
    with open(style_sheet_folder /"extra.css", "w") as file_pointer:
        file_pointer.write(textwrap.dedent(css_contents))